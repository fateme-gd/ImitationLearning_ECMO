...
KNOBS = ["PO2", "PCO2", "SpO2", "FiO2", "etCO2"]


#descale mlp file
MLP_MODEL_DIR = Path("models/mlp_models_20260217_015336")
if mlp_log_files and MLP_MODEL_DIR.exists():
    _scaler = joblib.load(MLP_MODEL_DIR / "scaler_full.pkl")
    with open(MLP_MODEL_DIR / "metadata.json") as _f:
        _meta = json.load(_f)
    _state_features = _meta[next(iter(_meta))]["state_features"]
    _n_state = len(_state_features)
    _mean = _scaler.mean_[:_n_state]
    _scale = _scaler.scale_[:_n_state]

    print(f"Descaling MLP ECE files using scaler from {MLP_MODEL_DIR} …")
    _new_mlp_log_files = []
    for fp in mlp_log_files:
        df = pd.read_csv(fp)
        csv_state_cols = [c for c in df.columns if c.startswith("state_")]
        if csv_state_cols == _state_features:
            df[_state_features] = df[_state_features].to_numpy() * _scale + _mean
            # Write to a temp descaled file so we don't modify originals
            out_path = fp.parent / fp.name.replace(".csv", "_descaled.csv")
            df.to_csv(out_path, index=False)
            _new_mlp_log_files.append(out_path)
            print(f"  ✓ {fp.name} → {out_path.name}")
        else:
            print(f"  ✗ {fp.name}: state column mismatch, skipping descale")
            _new_mlp_log_files.append(fp)

    mlp_log_files = _new_mlp_log_files
    print(f"  Done. MLP log files now point to descaled versions.\n")
